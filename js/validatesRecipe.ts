...
import { IRecipeForm } from "../models/recipe.interface.js";
  ...
  if (recipeForm.recipe.name.length > 0) {
    const recipeExists = categories.some((category) => {
      return category.recipes.some((recipe) => {
        return (
          recipe.name.toLowerCase() === recipeForm.recipe.name.toLowerCase()
        );
      });
    });
    return recipeExists;
...
export function checkRecipe(recipeForm: IRecipeForm): void {
  ...
  if (recipeForm.recipe.ingredients.length == 0) {
    ingredients.setAttribute(
      "oninvalid",
      `Insira os ingredientes necessários para preparar a sua receita!`
    );
  }

  if (recipeForm.recipe.methodOfPreparation.length == 0) {
    methodOfPreparation.setAttribute(
      "oninvalid",
      `Insira o modo de preparo da receita!`
    );
  }
